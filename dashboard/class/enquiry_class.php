...
class Enquiry extends metaModel {
	...
	{
		$case = "insert";
		$q = "Insert into Enquiry set name = '".$this->fix_for_mysqli($Name)."', email = '".$this->fix_for_mysqli($Email)."', contact_no = '".$this->fix_for_mysqli($ContactNo)."', flowtype = '".$this->fix_for_mysqli($Flowtype)."', type = '".$this->fix_for_mysqli($Type)."', category_id = '".$this->fix_for_mysqli($CategoryId)."', location_id = '".$this->fix_for_mysqli($LocationId)."', value1 = '".$this->fix_for_mysqli($Value1)."', value2 = '".$this->fix_for_mysqli($Value2)."', value3 = '".$this->fix_for_mysqli($Value3)."', modified_on = '".date('Y-m-d H:i:s')."', created_on = '".date('Y-m-d H:i:s')."',";
		$this->setQuery($q);
